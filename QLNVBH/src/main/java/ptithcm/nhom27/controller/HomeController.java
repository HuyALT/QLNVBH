...
//		@RequestParam("avatar") String avatar
	...
	@ModelAttribute("neededHours")
	public long getNeededHours() {
		NhanVienEntity nv = nvService.getNVLogin();
		LocalDate date = LocalDate.now();
		return lService.getNeededHoursOfEmployee_MONTH_YEAR(nv.getManv(),date.getMonthValue(), date.getYear());
	}
	
	@ModelAttribute("workedHours")
	public Float getWorkedHours() {
		NhanVienEntity nv = nvService.getNVLogin();
		LocalDate date = LocalDate.now();
		return ccService.getWorkedHoursOfEmployee_MONTH_YEAR(nv.getManv(), date.getMonthValue(), date.getYear());
	}
	
	@ModelAttribute("lateHours")
	public Float getLateHours() {
		NhanVienEntity nv = nvService.getNVLogin();
		LocalDate date = LocalDate.now();
		return ccService.getApointedHoursOfEmployee_MONTH_YEAR(nv.getManv(), date.getMonthValue(), date.getYear()) 
				-	ccService.getWorkedHoursOfEmployee_MONTH_YEAR(nv.getManv(),date.getMonthValue(), date.getYear());
	}
	
	@ModelAttribute("totalFine")
	public int getTotalFine() {
		NhanVienEntity nv = nvService.getNVLogin();
		LocalDate date = LocalDate.now();
		return lService.getFineOfEmployee_MONTH_YEAR(nv.getManv(), date.getMonthValue(), date.getYear());
	}
}
